...
import { ChevronLeft, ChevronRight, PackageSearch, AlertCircle, Loader2 } from "lucide-react";
  ...
  const fetchData = useCallback(
    async (page) => {
      ...
      try {
        ...
        const { products: productList, page: pg, pages, total } = normalizeProductsResponse(productsPayload);
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#F8F9FA]">
        ...
        <main className="flex-grow flex flex-col items-center justify-center gap-4 py-24 text-center px-4">
          ...
          <h1 className="text-2xl font-black text-gray-800">Category Not Found</h1>
          <p className="text-gray-500 text-sm max-w-sm">
            The category <span className="font-semibold text-gray-700">&ldquo;{slug}&rdquo;</span> doesn&apos;t
      ...
      <div className="flex flex-col min-h-screen bg-[#F8F9FA]">
        ...
        <main className="flex-grow flex flex-col items-center justify-center gap-4 py-24 text-center px-4">
          ...
          <button
            ...
            className="mt-2 inline-flex items-center gap-2 bg-[#ED1C24] hover:bg-[#d1171e] text-white text-sm font-bold px-5 py-2.5 rounded-lg transition-colors"
          >
            Try Again
          </button>
        </main>
      ...
      <main className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 lg:py-14">
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            <PackageSearch className="h-12 w-12 text-gray-300" />
            <h2 className="text-lg font-black text-gray-700">No Products Found</h2>
            <p className="text-gray-500 text-sm max-w-xs">
              There are no products in the <span className="font-semibold text-gray-700">{category.name}</span> category
              yet. Check back soon.
            </p>
            <Link
              href="/products"
